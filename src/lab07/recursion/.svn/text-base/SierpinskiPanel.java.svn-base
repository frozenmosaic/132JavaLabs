...
public class SierpinskiPanel extends JPanel {
	 ...
	 * 
	 * @param x
	 *            the x coordinate of the upper left corner of the triangle
	 * @param y
	 *            the y coordinate of the upper left corner of the triangle
	 * @param size
	 *            the width and height of the triangle
	 * @param g
	 *            the graphics object on which to draw the triangle
	 */
	...
	/**
	 * Draw a Sierpinski triangle of the specified order at the specified
	 * location on this JPanel.
	 * 
	 * @param order
	 *            the order of the Sierpinski triangle to draw.
	 * @param x
	 *            the x coordinate of the upper left corner of the central order
	 *            1 triangle
	 * @param y
	 *            the y coordinate of the upper left corner of the central order
	 *            1 triangle
	 * @param size
	 *            the width and height of the triangle
	 * @param g
	...
	private void drawSierpinski(int order, int x, int y, int size, Graphics g) {
		/*
		 * You'll want to make use of the drawTriangle method to do the actual
		...
		if (order == 1) {
			...
			drawSierpinski(1, x, y, size, g);
			drawSierpinski(order - 1, x - size / 4, y + size / 2, size / 2, g);
			drawSierpinski(order - 1, x + size / 4, y - size / 2, size / 2, g);
			drawSierpinski(order - 1, x + 3 * size / 4, y + size / 2, size / 2,
					g);
		}
	}

	/**
	 * Override the paintComponent method so that each time the JPanel is
	 * painted on the screen we can draw the Sierpinski Triangle on the JPanel.
	 * 
	 * @param g
	 *            the graphics object on which to draw the triangle.
	 */
	public void paintComponent(Graphics g) {
		...
		drawSierpinski(order, 205, 425, 200, g);
	}
